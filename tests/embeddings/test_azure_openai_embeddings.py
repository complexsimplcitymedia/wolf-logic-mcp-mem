...
from wolf-logic.embeddings.azure_ollama import AzureOpenAIEmbedding


@pytest.fixture
def mock_ollama_client():
    with patch("wolf-logic.embeddings.azure_ollama.AzureOpenAI") as mock_ollama:
        mock_client = Mock()
...
def test_embed_text(mock_ollama_client):
    ...
    mock_ollama_client.embeddings.create.return_value = mock_embedding_response
...
def test_init_with_api_key(monkeypatch, base_embedder_config):
    ...
    with (
        patch("wolf-logic.embeddings.azure_ollama.AzureOpenAI") as mock_azure_ollama,
        patch("wolf-logic.embeddings.azure_ollama.DefaultAzureCredential") as mock_cred,
        patch("wolf-logic.embeddings.azure_ollama.get_bearer_token_provider") as mock_token_provider,
    ):
...
def test_init_with_env_vars(monkeypatch, base_embedder_config):
    monkeypatch.setenv("EMBEDDING_AZURE_OLLAMA_URL", "env-key")
    ...
    with patch("wolf-logic.embeddings.azure_ollama.AzureOpenAI") as mock_azure_ollama:
        AzureOpenAIEmbedding(base_embedder_config)
        mock_azure_ollama.assert_called_once_with(
