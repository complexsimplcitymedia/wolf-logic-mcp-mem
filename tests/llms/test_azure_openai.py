...
from wolf-logic.configs.llms.azure import AzureOpenAIConfig
from wolf-logic.llms.azure_ollama import AzureOpenAILLM
...
def mock_ollama_client():
    with patch("wolf-logic.llms.azure_ollama.AzureOpenAI") as mock_ollama:
        mock_client = Mock()
...
def test_generate_response_without_tools(mock_ollama_client):
    ...
    mock_ollama_client.chat.completions.create.assert_called_once_with(
...
def test_generate_with_http_proxies(default_headers):
    ...
    with (
        patch("wolf-logic.llms.azure_ollama.AzureOpenAI") as mock_azure_ollama,
        ...
        config = AzureOpenAIConfig(
            ...
            top_p=TOP_P,
            api_key="test",
            http_client_proxies="http://testproxy.wolf-logic.net:8000",
            azure_kwargs=azure_kwargs,
        )

        _ = AzureOpenAILLM(config)

        mock_azure_ollama.assert_called_once_with(
            api_key="test",
        ...
        mock_http_client.assert_called_once_with(proxies="http://testproxy.wolf-logic.net:8000")


def test_init_with_api_key(monkeypatch):
    # Patch environment variables to None to force config usage
    monkeypatch.delenv("LLM_AZURE_OLLAMA_URL", raising=False)
    ...
    with patch("wolf-logic.llms.azure_ollama.AzureOpenAI") as mock_azure_ollama:
        llm = AzureOpenAILLM(config)
        mock_azure_ollama.assert_called_once_with(
...
def test_init_with_env_vars(monkeypatch):
    monkeypatch.setenv("LLM_AZURE_OLLAMA_URL", "env-key")
...
def test_init_with_default_azure_credential(monkeypatch):
    ...
    with (
        patch("wolf-logic.llms.azure_ollama.DefaultAzureCredential") as mock_cred,
        patch("wolf-logic.llms.azure_ollama.get_bearer_token_provider") as mock_token_provider,
        patch("wolf-logic.llms.azure_ollama.AzureOpenAI") as mock_azure_ollama,
    ):
