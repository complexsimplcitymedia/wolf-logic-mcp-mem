...
from wolf-logic.llms.azure_ollama_structured import SCOPE, AzureOpenAIStructuredLLM
...
class DummyAzureKwargs:
    def __init__(
        ...
        azure_endpoint="https://test-endpoint.ollama.azure.com",
...
@mock.patch("wolf-logic.llms.azure_ollama_structured.AzureOpenAI")
def test_init_with_api_key(mock_azure_ollama):
    ...
    mock_azure_ollama.assert_called_once()
    args, kwargs = mock_azure_ollama.call_args
    assert kwargs["api_key"] == "real-key"
    assert kwargs["azure_ad_token_provider"] is None


@mock.patch("wolf-logic.llms.azure_ollama_structured.AzureOpenAI")
@mock.patch("wolf-logic.llms.azure_ollama_structured.get_bearer_token_provider")
@mock.patch("wolf-logic.llms.azure_ollama_structured.DefaultAzureCredential")
def test_init_with_default_credential(mock_credential, mock_token_provider, mock_azure_ollama):
...
def test_init_with_env_vars(monkeypatch, mocker):
    mock_azure_ollama = mocker.patch("wolf-logic.llms.azure_ollama_structured.AzureOpenAI")
    monkeypatch.setenv("LLM_AZURE_DEPLOYMENT", "test-deployment")
    monkeypatch.setenv("LLM_AZURE_ENDPOINT", "https://test-endpoint.ollama.azure.com")
    ...
    assert kwargs["azure_endpoint"] == "https://test-endpoint.ollama.azure.com"
...
def test_init_with_placeholder_api_key_uses_default_credential(
    ...
    with (
        mock.patch("wolf-logic.llms.azure_ollama_structured.DefaultAzureCredential") as mock_credential,
        mock.patch("wolf-logic.llms.azure_ollama_structured.get_bearer_token_provider") as mock_token_provider,
    ):
