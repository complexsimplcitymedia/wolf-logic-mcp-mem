...
from wolf-logic.configs.llms.ollama import OpenAIConfig
from wolf-logic.llms.ollama import OpenAILLM
...
def test_ollama_llm_base_url():
    # case1: default config: with ollama official base url
    config = OpenAIConfig(model="gpt-4o", temperature=0.7, max_tokens=100, top_p=1.0, api_key="api_key")
    llm = OpenAILLM(config)
    # Note: ollama client will parse the raw base_url into a URL object, which will have a trailing slash
    assert str(llm.client.base_url) == "https://api.ollama.com/v1/"
    ...
    # case3: with config.ollama_base_url
    config_base_url = "https://api.config.com/v1"
    config = OpenAIConfig(
        model="gpt-4o", temperature=0.7, max_tokens=100, top_p=1.0, api_key="api_key", ollama_base_url=config_base_url
    )
    llm = OpenAILLM(config)
    # Note: ollama client will parse the raw base_url into a URL object, which will have a trailing slash
...
def test_response_callback_invocation(mock_ollama_client):
    # Setup mock callback
    mock_callback = Mock()
...
def test_no_response_callback(mock_ollama_client):
...
def test_callback_exception_handling(mock_ollama_client):
...
def test_callback_with_tools(mock_ollama_client):
    mock_callback = Mock()
    config = OpenAIConfig(model="gpt-4o", response_callback=mock_callback)
    llm = OpenAILLM(config)
