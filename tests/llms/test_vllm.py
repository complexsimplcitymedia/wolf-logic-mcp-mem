...
from wolf-logic import AsyncMemory, Memory
from wolf-logic.configs.llms.base import BaseLlmConfig
from wolf-logic.llms.vllm import VllmLLM


@pytest.fixture
def mock_vllm_client():
    with patch("wolf-logic.llms.vllm.OpenAI") as mock_ollama:
        mock_client = Mock()
...
def create_mocked_memory():
    ...
    with patch('wolf-logic.utils.factory.LlmFactory.create') as mock_llm_factory, \
         patch('wolf-logic.utils.factory.EmbedderFactory.create') as mock_embedder_factory, \
         patch('wolf-logic.utils.factory.VectorStoreFactory.create') as mock_vector_factory, \
         patch('wolf-logic.memory.storage.SQLiteManager') as mock_sqlite:
...
async def test_async_thinking_tags_async():
    ...
    with patch('wolf-logic.memory.main.asyncio.to_thread', side_effect=mock_to_thread):
