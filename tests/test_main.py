...
from wolf-logic.configs.base import MemoryConfig
from wolf-logic.memory.main import Memory


@pytest.fixture(autouse=True)
def mock_ollama():
    os.environ["OLLAMA_URL"] = "123"
    with patch("ollama.OpenAI") as mock:
...
def memory_instance():
    with (
        patch("wolf-logic.utils.factory.EmbedderFactory") as mock_embedder,
        patch("wolf-logic.memory.main.VectorStoreFactory") as mock_vector_store,
        patch("wolf-logic.utils.factory.LlmFactory") as mock_llm,
        patch("wolf-logic.memory.telemetry.capture_event"),
        patch("wolf-logic.memory.graph_memory.MemoryGraph"),
        patch("wolf-logic.memory.main.GraphStoreFactory") as mock_graph_store,
    ):
...
def test_custom_prompts(memory_custom_instance):
    ...
    from wolf-logic.embeddings.mock import MockEmbeddings
    ...
    with patch("wolf-logic.memory.main.parse_messages", return_value="Test message") as mock_parse_messages:
        with patch(
            "wolf-logic.memory.main.get_update_memory_messages", return_value="custom update memory prompt"
