...
from wolf-logic.configs.vector_stores.azure_ai_search import AzureAISearchConfig

# Import the AzureAISearch class and related models
from wolf-logic.vector_stores.azure_ai_search import AzureAISearch
...
def mock_clients():
    with (
        patch("wolf-logic.vector_stores.azure_ai_search.SearchClient") as MockSearchClient,
        patch("wolf-logic.vector_stores.azure_ai_search.SearchIndexClient") as MockIndexClient,
        patch("wolf-logic.vector_stores.azure_ai_search.AzureKeyCredential") as MockAzureKeyCredential,
    ):
...
def test_config_validation_valid():
    ...
    assert config.collection_name == "wolf-logic"  # Default value
...
def test_initialization(mock_clients):
    ...
    assert "wolf-logic" in mock_search_client._client._config.user_agent_policy.add_user_agent.call_args[0]
    assert "wolf-logic" in mock_index_client._client._config.user_agent_policy.add_user_agent.call_args[0]
...
def test_init_with_valid_api_key(mock_clients):
    ...
    mock_search_client._client._config.user_agent_policy.add_user_agent.assert_called_with("wolf-logic")
    mock_index_client._client._config.user_agent_policy.add_user_agent.assert_called_with("wolf-logic")
...
def test_init_with_default_api_key_triggers_default_credential(monkeypatch, mock_clients):
    ...
    with patch("wolf-logic.vector_stores.azure_ai_search.DefaultAzureCredential") as mock_default_cred:
