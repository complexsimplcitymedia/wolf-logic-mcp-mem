...
from wolf-logic.vector_stores.databricks import Databricks
import pytest
...
def mock_workspace_client():
    ...
    with patch("wolf-logic.vector_stores.databricks.WorkspaceClient") as mock_wc_cls:
        ...
        mock_wc.vector_search_indexes.create_index.return_value = SimpleNamespace(name="catalog.schema.wolf-logic")
        ...
        mock_wc.vector_search_indexes.get_index.return_value = SimpleNamespace(name="wolf-logic")
...
def test_list_cols(db_instance_delta, mock_workspace_client):
    mock_workspace_client.vector_search_indexes.list_indexes.return_value = [
        SimpleNamespace(name="catalog.schema.wolf-logic"),
        SimpleNamespace(name="catalog.schema.other"),
    ]
    cols = db_instance_delta.list_cols()
    assert "catalog.schema.wolf-logic" in cols and "catalog.schema.other" in cols


def test_col_info(db_instance_delta):
    info = db_instance_delta.col_info()
    assert info["name"] == "wolf-logic"
