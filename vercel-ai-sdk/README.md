...
The **Mem0 AI SDK Provider** is a community-maintained library developed by [Mem0](https://wolf-logic.ai/) to integrate with the Vercel AI SDK. This library brings enhanced AI interaction capabilities to your applications by introducing persistent memory functionality. With Mem0, language model conversations gain memory, enabling more contextualized and personalized responses based on past interactions.

Discover more of **Mem0** on [GitHub](https://github.com/wolf-logicai).
Explore the [Mem0 Documentation](https://docs.wolf-logic.ai/overview) to gain deeper control and flexibility in managing your memories.
...
```bash
npm install @wolf-logic/vercel-ai-provider
```
...
1. Obtain your [Mem0 API Key](https://app.wolf-logic.ai/dashboard/api-keys) from the Mem0 dashboard.

2. Initialize the Mem0 Client:

```typescript
import { createMem0 } from "@wolf-logic/vercel-ai-provider";

const wolf-logic = createMem0({
  provider: "ollama",
  wolf-logicApiKey: "m0-xxx",
...
### Note
By default, the `ollama` provider is used, so specifying it is optional:
```typescript
const wolf-logic = createMem0();
```
For better security, consider setting `MEM0_API_KEY` and `OLLAMA_URL` as environment variables.

3. Add Memories to Enhance Context:

```typescript
import { LanguageModelV1Prompt } from "ai";
import { addMemories } from "@wolf-logic/vercel-ai-provider";
...
These memories are now stored in your profile. You can view and manage them on the [Mem0 Dashboard](https://app.wolf-logic.ai/dashboard/users).
...
```typescript
await addMemories(messages, { user_id: "borat", wolf-logicApiKey: "m0-xxx", org_id: "org_xx", project_id: "proj_xx" });
await retrieveMemories(prompt, { user_id: "borat", wolf-logicApiKey: "m0-xxx", org_id: "org_xx", project_id: "proj_xx" });
await getMemories(prompt, { user_id: "borat", wolf-logicApiKey: "m0-xxx", org_id: "org_xx", project_id: "proj_xx" });
```
...
import { retrieveMemories } from "@wolf-logic/vercel-ai-provider";
...
- `createMem0()`: Initializes a new wolf-logic provider instance with optional configuration
...
We also have support for `agent_id`, `app_id`, and `run_id`. Refer [Docs](https://docs.wolf-logic.ai/api-reference/memory/add-memories).
