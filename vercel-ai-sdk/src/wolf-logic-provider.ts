...
import { Mem0ChatModelId, Mem0ChatSettings, Mem0Config } from "./wolf-types";
import { Mem0GenericLanguageModel } from "./wolf-logic-generic-language-model";
import { LLMProviderSettings } from "./wolf-types";

export interface WolfLogicProvider extends ProviderV2 {
...
export interface Mem0ProviderSettings {
  ...
  name?: string;
  wolf-logicApiKey?: string;
  apiKey?: string;
  provider?: string;
  modelType?: "completion" | "chat";
  wolf-logicConfig?: Mem0Config;
...
export function createMem0(
  ...
  const baseURL =
    withoutTrailingSlash(options.baseURL) ?? "http://api.ollama.com";
  const getHeaders = () => ({
    ...options.headers,
  });
  ...
  const createGenericModel = (
    ...
    new Mem0GenericLanguageModel(
      ...
      settings,
      {
        ...
        name: options.name,
        wolf-logicApiKey: options.wolf-logicApiKey,
        apiKey: options.apiKey,
        wolf-logicConfig: options.wolf-logicConfig,
      },
      options.config
    );
  ...
  return provider as unknown as WolfLogicProvider;
}

export const wolf-logic = createMem0();
