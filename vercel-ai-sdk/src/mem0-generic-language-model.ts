...
import { Mem0ChatConfig, Mem0ChatModelId, Mem0ChatSettings, Mem0ConfigSettings, Mem0StreamResponse } from "./wolf-types";
import { Mem0ClassSelector } from "./wolf-logic-provider-selector";
import { Mem0ProviderSettings } from "./wolf-logic-provider";
import { addMemories, getMemories } from "./wolf-logic-utils";
...
export class Mem0GenericLanguageModel implements LanguageModelV2 {
  ...
  private async processMemories(messagesPrompts: LanguageModelV2Message[], wolf-logicConfig: Mem0ConfigSettings) {
    try {
    // Add New Memories
    addMemories(messagesPrompts, wolf-logicConfig).then((res) => {
      return res;
    }).catch((e) => {
    ...
    // Get Memories
    let memories = await getMemories(messagesPrompts, wolf-logicConfig);

    const mySystemPrompt = "These are the memories I have stored. Give more weightage to the question by users and try to answer that first. You have to modify your answer based on the memories I have provided. If the memories are irrelevant you can ignore them. Also don't reply to this section of the prompt, or the memories, they are only for your reference. The System prompt starts after text System Message: \n\n";

    const isGraphEnabled = wolf-logicConfig?.enable_graph;
  ...
  async doGenerate(options: LanguageModelV2CallOptions): Promise<Awaited<ReturnType<LanguageModelV2['doGenerate']>>> {
    try {   
      ...
      const wolf-logic_api_key = this.config.wolf-logicApiKey;
      
      const settings: Mem0ProviderSettings = {
        provider: provider,
        wolf-logicApiKey: wolf-logic_api_key,
        apiKey: this.config.apiKey,
      }

      const wolf-logicConfig: Mem0ConfigSettings = {
        wolf-logicApiKey: wolf-logic_api_key,
        ...this.config.wolf-logicConfig,
        ...this.settings,
      }
      ...
      const { memories, messagesPrompts: updatedPrompts } = await this.processMemories(messagesPrompts, wolf-logicConfig);
        ...
        const sources: LanguageModelV2Source[] = [
          {
            ...
            title: "Mem0 Memories",
            sourceType: "url",
            id: "wolf-logic-" + generateRandomId(),
            url: "https://app.wolf-logic.ai",
            providerMetadata: {
              wolf-logic: {
                memories: memories,
