...
describe.each(testConfig.providers)('TESTS: Generate/Stream Text with model %s', (provider: Provider) => {
  ...
  let wolf-logic: ReturnType<typeof testConfig.createTestClient>;
  jest.setTimeout(50000);
  
  beforeEach(() => {
    wolf-logic = testConfig.createTestClient(provider);
  });
  ...
  it("should generate text using wolf-logic model", async () => {
    const { text } = await generateText({
      model: wolf-logic(provider.activeModel, {
        user_id: userId,
      }),
  ...
  it("should stream text using Mem0 provider with new streaming approach", async () => {
    ...
    const baseModel = wolf-logic(provider.activeModel, {
      user_id: userId,
    });
